...
active unified name. The client processes pending complete key sets in
`update` when it is created, then reads from `current` and `history` using the
...
crypto.IsEncrypted(encryptedValue);
...
Applications that use the module do not need `ICryptoClient` in order to use

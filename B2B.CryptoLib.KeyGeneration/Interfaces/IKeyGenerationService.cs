...
namespace B2B.CryptoLib.KeyGeneration.Interfaces
{
    ...
    /// </summary>
    /// <remarks>
    /// 實作會呼叫對應產生器；<see cref="GenerateKeyOnly{TModel}(CryptoAlgorithmType)"/>
    /// 不寫檔，而 <see cref="GenerateAndSaveKey{TModel}(CryptoAlgorithmType, string?)"/>
    /// 會把私密金鑰與其他金鑰材料寫入由 <see cref="B2B.CryptoLib.Config.CryptoConfig"/>
    /// 決定的目錄。此介面不得註冊到 WebAPI 執行階段。
    /// </remarks>
    public interface IKeyGenerationService
    {
        /// <summary>只在記憶體中產生一組金鑰。</summary>
        /// <typeparam name="TModel">要產生的金鑰模型類型。</typeparam>
        /// <param name="algorithm">要產生的演算法。</param>
        /// <returns>新產生的金鑰模型。</returns>

...
namespace B2B.CryptoLib.Helpers
{
    ...
    /// </summary>
    /// <remarks>Hex 僅是可讀的 bytes 表示法，不提供加密、完整性或秘密保護。</remarks>
    public static class HexUtils
    {
        /// <summary>將非空 bytes 編碼為連續的十六進位字串。</summary>
        /// <param name="data">要編碼的資料。</param>
        /// <param name="upperCase">true 產生大寫 A-F；false 產生小寫 a-f。</param>
        /// <returns>每個 byte 兩個字元的 Hex 字串。</returns>
        ...
        /// <summary>將偶數長度 Hex 字串解碼為 bytes。</summary>
        /// <param name="hex">每兩個字元代表一個 byte 的字串。</param>

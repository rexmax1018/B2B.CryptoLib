...
namespace B2B.CryptoLib.Config
{
    ...
    /// </summary>
    /// <remarks>
    /// <see cref="CryptoConfig"/> 是舊版、程序層級的設定入口，主要供離線
    /// KeyGeneration 與 KeyGenTool 使用。執行階段的 <see cref="CryptoClient"/> 與
    /// <see cref="Crypto"/> 不會自動讀取這個狀態；多個測試或租戶需要隔離設定時，
    /// 請使用 <see cref="Models.CryptoOptions"/> 建立執行個體專屬的執行脈絡。
    /// <para>
    /// 這個靜態快取沒有提供並行變更協定；宿主應在啟動或工具工作開始前
    ...
    public static class CryptoConfig
    {
        ...
        /// </summary>
        /// <param name="jsonPath">相對於應用程式基底目錄的檔案路徑；絕對路徑會先經 <see cref="Path.Combine(string, string)"/> 規則處理。</param>
        ...
        /// 呼叫會覆寫程序層級的 <see cref="Current"/>；不會建立金鑰目錄，
        /// 也不會觸發金鑰產生或發布。
        /// </remarks>
        ...
        /// </summary>
        /// <param name="model">要成為 <see cref="Current"/> 的設定模型；傳入 <see langword="null"/> 會清除目前快取。</param>
        /// <remarks>
        ...
        /// <returns>由密碼學安全的隨機位元組選出的八碼英數字串，再加上 <paramref name="extension"/>。</returns>
        /// <remarks>
        /// 此名稱只用於降低離線工具檔名碰撞機率，不是金鑰材料，也不是安全授權權杖。
        /// </remarks>

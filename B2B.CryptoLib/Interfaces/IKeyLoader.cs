...
namespace B2B.CryptoLib.Interfaces
{
    ...
    /// </summary>
    /// <remarks>
    /// loader 會把序列化內容轉成記憶體中的 model；它不負責路徑安全、金鑰輪替、
    /// permissions 或秘密清除。<see cref="LoadFromStream(Stream)"/> 會消費目前 stream
    /// 的文字內容，呼叫端應依實作契約管理 stream 的生命週期。
    /// </remarks>
    public interface IKeyLoader<T> where T : class
    {
        /// <summary>從檔案讀取並反序列化 key model。</summary>
        ...
        /// <exception cref="System.IO.InvalidDataException">檔案不是有效的 model 內容。</exception>
        T LoadFromFile(string path);

        /// <summary>從序列化文字讀取 key model。</summary>
        ...
        /// <summary>從 Base64 編碼的序列化文字讀取 key model。</summary>
        ...
        /// <summary>從目前位置讀取資料流並反序列化 key model。</summary>
        /// <param name="stream">包含 UTF-8 序列化內容且可讀的 stream。</param>
        /// <returns>反序列化後的 <typeparamref name="T"/>。</returns>

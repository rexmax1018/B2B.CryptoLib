...
namespace B2B.CryptoLib.Interfaces
{
    ...
    /// </summary>
    /// <remarks>
    /// 每個 instance 都有自己的金鑰根目錄、active unified name 與 cache；它不會
    /// 使用 <see cref="B2B.CryptoLib.Config.CryptoConfig"/> 的 process-wide 設定。建構完成後不會
    /// 自動處理 <c>update</c> 目錄；請顯式呼叫 <see cref="UpdateKeySetsAsync"/>。
    /// <para>
    /// 相同 instance 的查找與更新由內部鎖序列化，讓更新完成前不會把半組檔案交給
    /// 同一個 client；但不同 instance 或不同 process 沒有跨 context 協調鎖。
    /// </para>
    /// </remarks>
    public interface ICryptoClient
    {
        /// <summary>使用建立 client 時設定的 active unified name 加密文字。</summary>
        /// <param name="plainText">要加密的文字；null 或空字串回傳 null。</param>
        /// <returns>格式為 <c>Base64(payload).unifiedName</c> 的密文，或空輸入的 null。</returns>
        /// <exception cref="System.InvalidOperationException">未設定 active name，或找不到對應的完整 key set。</exception>
        string? Encrypt(string? plainText);

        /// <summary>使用明確 unified name 加密文字。</summary>
        /// <param name="plainText">要加密的文字；null 或空字串回傳 null。</param>
        /// <param name="unifiedName">要使用的 key-set 名稱；不可為空且不可包含句點。</param>
        /// <returns>格式為 <c>Base64(payload).unifiedName</c> 的密文，或空輸入的 null。</returns>
        /// <exception cref="System.ArgumentException">名稱無效，或無法找到完整 key set。</exception>
        string? Encrypt(string? plainText, string? unifiedName);

        /// <summary>解密包含 unified name 尾綴的文字密文。</summary>
        ...
        /// <exception cref="System.ArgumentException">外層格式或 unified name 尾綴無效。</exception>
        ...
        /// <returns>外層格式可解析時為 true；不保證 tag、AAD、金鑰或授權。</returns>
        bool IsValidEncryptedFormat(string? data);

        /// <summary>從密文尾綴擷取 unified name。</summary>
        /// <param name="encryptedDataWithUnifiedName">包含 payload 與名稱尾綴的密文。</param>
        /// <returns>尾綴中的名稱；null 或空輸入回傳 null。</returns>
        /// <exception cref="System.ArgumentException">缺少非空 payload 或名稱尾綴。</exception>
        string? GetUnifiedName(string? encryptedDataWithUnifiedName);

        /// <summary>取得 unified name 的相容命名版本。</summary>
        /// <param name="encryptedDataWithUnifiedName">包含 payload 與名稱尾綴的密文。</param>
        ...
        /// <summary>明確執行一次 update key-set 掃描、發布與 cache invalidation。</summary>
        /// <returns>更新完成後完成的工作。</returns>
        /// <remarks>這是唯一由高階 client 觸發檔案發布的生命週期操作；建立 client 不會自動執行。</remarks>

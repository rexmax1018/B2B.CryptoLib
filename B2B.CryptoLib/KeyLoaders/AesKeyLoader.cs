...
namespace B2B.CryptoLib.KeyLoaders
{
    ...
    /// </summary>
    /// <remarks>
    /// loader 預期 UTF-8 JSON，其欄位可還原為 <see cref="SymmetricKeyModel"/> 的
    /// Key 與 IV。它只解析資料，不驗證金鑰長度；長度檢查會在實際密碼運算時發生。
    /// </remarks>
    public class AesKeyLoader : IKeyLoader<SymmetricKeyModel>
    {
        /// <summary>從 UTF-8 JSON 檔案載入 AES model。</summary>
        ...
        /// <exception cref="InvalidDataException">JSON 無法轉為 AES model。</exception>
        public SymmetricKeyModel LoadFromFile(string path) => Deserialize(File.ReadAllText(path));

        /// <summary>從 UTF-8 JSON 文字載入 AES model。</summary>
        ...
        /// <summary>從包含 UTF-8 JSON 的標準 Base64 字串載入 AES model。</summary>
        ...
        /// <summary>讀取資料流剩餘內容並載入 AES model。</summary>
        ...
        /// <remarks>方法會讀到 stream 結尾並因 <see cref="StreamReader"/> 的 using 而關閉傳入 stream。</remarks>
        ...
        private static SymmetricKeyModel Deserialize(string json)
        {
            try
            {
                // Keep JSON loading focused on the persisted model shape; key
                // length enforcement belongs to the crypto primitive boundary.

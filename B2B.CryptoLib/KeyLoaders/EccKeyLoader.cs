...
namespace B2B.CryptoLib.KeyLoaders
{
    ...
    public class EccKeyLoader : IKeyLoader<EccKeyModel>
    {
        ...
        /// <exception cref="InvalidDataException">JSON 無法轉為 ECC model。</exception>
        public EccKeyModel LoadFromFile(string path) => Deserialize(File.ReadAllText(path));

        /// <summary>從 UTF-8 JSON 文字載入 ECC model。</summary>
        ...
        /// <summary>從包含 UTF-8 JSON 的標準 Base64 字串載入 ECC model。</summary>
        ...
        /// <summary>讀取資料流剩餘內容並載入 ECC model。</summary>
        ...
        // Keep the serialized PEM and curve metadata untouched so existing
        // signature consumers retain their established key representation.
